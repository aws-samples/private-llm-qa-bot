...
class LLMInputOutputAdapter:
    ...
    def prepare_output_stream(
        ...
        # print('---prepare_output_stream---messages_api:',messages_api)
        if not stream:
            return
        ...
        for event in stream:
            ...
            if messages_api and chunk_obj.get("type") in (
                ...
                ## 取出usage token信息
                if messages_api and chunk_obj.get("type") == "message_stop":
                    chk =  GenerationChunk(
                            text=json.dumps(chunk_obj["amazon-bedrock-invocationMetrics"]),
                            generation_info=dict(
                                finish_reason=chunk_obj.get("stop_reason", "end_turn"),
                            ))
                    yield chk
                # chunk obj format varies with provider
                elif chunk_obj.get("type")  not in ["message_stop","content_block_stop","message_delta"]: 
...
class BedrockBase(BaseModel, ABC):
    ...
    def _prepare_input_and_invoke(
        ...
        **kwargs: Any,
        ...
        # print('-----_prepare_input_and_invoke()--------')
        try:
    ...
    def _prepare_input_and_invoke_stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ValueError(f"Error raised by bedrock service: {e}")
        # print('------_prepare_input_and_invoke_stream()---------\n\n')
        # print(response)

        for chunk in LLMInputOutputAdapter.prepare_output_stream(
            provider, response, stop, True if messages else False
        ):
            if not chunk.generation_info.get('finish_reason') == 'end_turn':
                yield chunk
            # print(chunk,'\n')
            ...
            if run_manager is not None and chunk.generation_info.get('finish_reason') != 'end_turn':
                run_manager.on_llm_new_token(chunk.text, chunk=chunk)
            
            if run_manager is not None and chunk.generation_info.get('finish_reason') == 'end_turn':
                run_manager.on_llm_end(None,invocationMetrics=chunk.text)
    ...
    async def _aprepare_input_and_invoke_stream(
        ...
        **kwargs: Any,
        ...
        body = json.dumps(input_body)
        # print('-----async-_prepare_input_and_invoke_stream()---------\n\n')
