...
def _anthropic_messages_format(input_text: str) -> str:
    messages = []
    # single-round conversation
    if input_text.count("Human:") <= 1:
        input_text = input_text.replace('Human:', '').replace('Assistant:', '')
        message = {"role": "user", "content": input_text}
        messages.append(message)
        return messages
    else: # multi-round conversation
        rounds = input_text.split('"Human:"')
        for round_str in rounds:
            talk = round_str.split('"Assistant:"',2)
    ...
    def prepare_input(
        ...
        if provider == "anthropic":
            ...
            try:
                ...
                input_body["messages"] = _anthropic_messages_format(prompt)
                input_body["max_tokens"] = model_kwargs.get('max_tokens', model_kwargs.get('max_tokens_to_sample', 256))
