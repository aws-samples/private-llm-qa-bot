...
func_image=price_server
funcimage_fullname=${account}.dkr.ecr.${region}.amazonaws.${suffix}/${func_image}:latest

# If the repository doesn't exist in ECR, create it.
aws  ecr describe-repositories --repository-names "${func_image}" --region ${region} || aws  ecr create-repository --repository-name "${func_image}" --region ${region}

if [ $? -ne 0 ]
then
    aws  ecr create-repository --repository-name "${func_image}" --region ${region}
fi
