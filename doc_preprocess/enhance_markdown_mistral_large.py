...
def construct_gen_summary_prompt(content):
    ...
    prompt = f"""<s>{systemPrompt}[Inst]Please give a summary for below content.
    <content>
    {content}
    <content>

    Please skip the preamble, go straight into the answer. Put the answer in <summary></summary>.[/Inst]
...
def construct_QA_list_prompt(content, NUMBER=5):
    ...
    prompt = f"""<s>${systemPrompt}[Inst]Utilize Natural Language Processing techniques and Generative AI to create new Question/Answer pair textual training data for OpenAI LLMs by drawing inspiration from the given seed content: <content>{content}</content> 
...
Only output the resulting dataframe in the format of this example:  <output>{examples}</output>[/Inst]
...
def call_bedrock_enhance(content, prompt_func):
    ...
    result = body_dict['outputs'][0].get("text")
        ...
        for root, dirs, files in os.walk(input_path):
            ...
            for filename in tqdm(md_files):
                ...
                if df is not None:
                    ...
                    df.to_excel(f'{output_path}/{output_filename}', index=False)
    else:      
        s3 = boto3.client('s3')
